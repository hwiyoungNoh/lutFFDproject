...
#include <algorithm> // min, max 함수를 사용하기 위해 추가
...
// 12단계, 8단계, 16단계를 65단계 인덱스로 변환하는 함수
...
// 주어진 포인트와 인접한 포인트 사이에 포함되는 65단계 인덱스를 추출하는 함수
vector<tuple<int, int, int>> findIncluded65Indices(int h1, int s1, int v1, int h2, int s2, int v2) {
    auto point1_65 = convertTo65Index(h1, s1, v1);
    auto point2_65 = convertTo65Index(h2, s2, v2);

    int h1_65, s1_65, v1_65;
    int h2_65, s2_65, v2_65;
    tie(h1_65, s1_65, v1_65) = point1_65;
    tie(h2_65, s2_65, v2_65) = point2_65;

    vector<int> h_indices, s_indices, v_indices;
    for (int h = min(h1_65, h2_65); h <= max(h1_65, h2_65); ++h) {
        h_indices.push_back(h);
    }
    for (int s = min(s1_65, s2_65); s <= max(s1_65, s2_65); ++s) {
        s_indices.push_back(s);
    }
    for (int v = min(v1_65, v2_65); v <= max(v1_65, v2_65); ++v) {
        v_indices.push_back(v);
    }

    vector<tuple<int, int, int>> included65Indices;
    for (int h : h_indices) {
        for (int s : s_indices) {
            for (int v : v_indices) {
                included65Indices.emplace_back(h, s, v);
            }
        }
    }

    return included65Indices;
}

// 상대 거리를 계산하는 함수
double calculateRelativeDistance(int h1, int s1, int v1, int h2, int s2, int v2, int h, int s, int v) {
    double totalDistance = calculateDistance(h1, s1, v1, h2, s2, v2);
    double currentDistance = calculateDistance(h1, s1, v1, h, s, v);
...
int main() {
    ...
    // 주어진 포인트와 인접한 포인트 사이에 포함되는 65단계 인덱스 추출 및 출력
    cout << "주어진 포인트와 인접한 포인트 사이에 포함되는 65단계 인덱스들:" << endl;
    for (const auto& point : adjacentPoints12_8_16) {
        int adjH12, adjS8, adjV16;
        tie(adjH12, adjS8, adjV16) = point;
        auto includedIndices = findIncluded65Indices(h, s, v, adjH12, adjS8, adjV16);
        double distance = calculateDistance(h, s, v, adjH12, adjS8, adjV16);
        cout << "12단계 인덱스 (H=" << adjH12 << ", S=" << adjS8 << ", V=" << adjV16 << ") 사이의 포함된 65단계 인덱스들 (거리=" << distance << "):" << endl;
        cout << setw(5) << "H" << setw(5) << "S" << setw(5) << "V" << setw(10) << "상대 거리" << endl;
        for (const auto& index : includedIndices) {
            int h65, s65, v65;
            tie(h65, s65, v65) = index;
            double relativeDistance = calculateRelativeDistance(h, s, v, adjH12, adjS8, adjV16, h65, s65, v65);
            cout << setw(5) << h65 << setw(5) << s65 << setw(5) << v65 << setw(10) << fixed << setprecision(2) << relativeDistance << endl;
        }
    }
