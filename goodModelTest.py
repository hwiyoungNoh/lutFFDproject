...
angle_deformations = []
for i in range(num_layers):
    angle_deformation = 0.1 * np.sin(np.linspace(0, 2*np.pi, num_axes, endpoint=False) + i * np.pi / num_layers)
    angle_deformations.append(angle_deformation)
angle_deformations = np.array(angle_deformations)
...
def smooth_deform_point_3d_interpolated(x, y, z, axis_angles, deformations_3d, angle_deformations, level_spacings, level_ranges, layer_zs, sharpness=8):
    ...
    angle_deformation_low = np.dot(weights, angle_deformations[idx_low])
    angle_deformation_high = np.dot(weights, angle_deformations[idx_high])
    angle_deformation = linear_interp_z(z, layer_zs[idx_low], layer_zs[idx_high], angle_deformation_low, angle_deformation_high)
    ...
    theta_new = theta + angle_deformation
    x_new = center + r_new * np.cos(theta_new)
    y_new = center + r_new * np.sin(theta_new)
    z_new = z
...
# 특정 제어점을 반대 방향으로 이동
def move_control_point(grid_points, level, axis, distance):
    x, y = grid_points[level, axis]
    dx, dy = x - center, y - center
    r = np.sqrt(dx ** 2 + dy ** 2)
    theta = np.arctan2(dy, dx) % (2 * np.pi)
    theta_new = (theta + np.pi) % (2 * np.pi)  # 반대 방향으로 이동
    x_new = center + (r + distance) * np.cos(theta_new)
    y_new = center + (r + distance) * np.sin(theta_new)
    grid_points[level, axis] = (x_new, y_new)
    return grid_points

# 예를 들어, 첫 번째 레벨의 첫 번째 축의 제어점을 반대 방향으로 이동
level_to_change = 0
axis_to_change = 0
distance_to_move = 2.0
grid_points = move_control_point(grid_points, level_to_change, axis_to_change, distance_to_move)
...
for i, z0 in enumerate(layer_zs):
    for l in range(num_levels):
        ...
        for j in range(num_axes):
            ...
            x, y, z__, _ = smooth_deform_point_3d_interpolated(x_base, y_base, z0, axis_angles, deformations_3d, angle_deformations, level_spacings, level_ranges, layer_zs, sharpness)
            ring.append((x, y, z0))
        ring = np.array(ring)
...
# 2D 시각화: 각 z 단면별 그래프
fig2, axes2d = plt.subplots(1, num_layers, figsize=(20, 4))
for i, z0 in enumerate(layer_zs):
    ax = axes2d[i]
    ax.set_title(f'z = {z0:.1f}')
    ax.set_xlim(0, grid_size)
    ax.set_ylim(0, grid_size)
    ax.set_aspect('equal')
    # 변형된 grid 데이터
    layer_points = deformed_points[deformed_points[:, 2] == z0]
    ax.scatter(layer_points[:, 0], layer_points[:, 1], c=layer_points[:, 2], cmap='cool', s=3, alpha=0.4)
    for l in range(num_levels):
        ring = []
        for j in range(num_axes):
            x_base, y_base = grid_points[l, j]
            ...
            ring.append((x, y))
        ring = np.array(ring)
        ax.plot(np.append(ring[:,0], ring[0,0]), np.append(ring[:,1], ring[0,1]), ls='--', color=cmap(l/num_levels), lw=2)
    # 제어점(가장 바깥 ring, 변형된 좌표) 별도 강조
    out_ring = []
    for j in range(num_axes):
        x_base, y_base = grid_points[-1, j]
        x, y, z__, _ = smooth_deform_point_3d_interpolated(x_base, y_base, z0, axis_angles, deformations_3d, angle_deformations, level_spacings, level_ranges, layer_zs, sharpness)
        out_ring.append((x, y))
    out_ring = np.array(out_ring)
    ax.scatter(out_ring[:,0], out_ring[:,1], color='k', s=70, marker='o', alpha=0.7)
    ax.plot(np.append(out_ring[:,0], out_ring[0,0]), np.append(out_ring[:,1], out_ring[0,1]), color='k', lw=3, alpha=0.7)
